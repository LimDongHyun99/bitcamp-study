...
CREATE TABLE member (
  ...
  status      BOOLEAN      NOT NULL DEFAULT true COMMENT '상태' -- 상태
)
COMMENT '회원';
...
CREATE TABLE team_field_member (
  ...
  cdt   DATE        NOT NULL DEFAULT now() COMMENT '등록일', -- 등록일
  type  VARCHAR(20) NOT NULL DEFAULT '미승인' COMMENT '상태' -- 상태
)
COMMENT '팀원모집분야지원';
...
CREATE TABLE product_order (
  ...
  scno        INTEGER      NOT NULL DEFAULT 1 COMMENT '주문상태카테고리번호' -- 주문상태카테고리번호
)
COMMENT '주문내역';
