...
-- FAQ신고
DROP TABLE IF EXISTS faqreport RESTRICT;
...
CREATE TABLE faqreport (
  repno   INTEGER    NOT NULL COMMENT '신고번호', -- 신고번호
  mno     INTEGER    NOT NULL COMMENT '회원번호', -- 회원번호
  faqno   INTEGER    NOT NULL COMMENT 'FAQ번호', -- FAQ번호
  repcont MEDIUMTEXT NOT NULL COMMENT '신고내용', -- 신고내용
  repcdt  DATE       NOT NULL DEFAULT now() COMMENT '신고시간' -- 신고시간
)
COMMENT 'FAQ신고';

-- FAQ신고
ALTER TABLE faqreport
  ADD CONSTRAINT PK_faqreport -- FAQ신고 기본키
    PRIMARY KEY (
      repno -- 신고번호
    );

ALTER TABLE faqreport
  MODIFY COLUMN repno INTEGER NOT NULL AUTO_INCREMENT COMMENT '신고번호';
...
-- FAQ신고
ALTER TABLE faqreport
  ADD CONSTRAINT FK_member_TO_faqreport -- 회원 -> FAQ신고
    FOREIGN KEY (
      mno -- 회원번호
    )
    REFERENCES member ( -- 회원
...
-- FAQ신고
ALTER TABLE faqreport
  ADD CONSTRAINT FK_faq_TO_faqreport -- FAQ -> FAQ신고
    FOREIGN KEY (
      faqno -- FAQ번호
    )
    REFERENCES faq ( -- FAQ
